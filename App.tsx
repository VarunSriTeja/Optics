...
const App: React.FC = () => {
  ...
  const navigateToSection = (id: string) => {
    // Prevent navigating away during active experiment phases
    if (phase === ExperimentPhase.STARING || phase === ExperimentPhase.PERSISTENCE) return;
    
    // Switch to IDLE so the theory sections are rendered in the DOM
    setPhase(ExperimentPhase.IDLE);
    
    // Small timeout to allow React to render the IDLE view before scrolling
    setTimeout(() => {
        ...
        {phase === ExperimentPhase.IDLE || phase === ExperimentPhase.CONFIGURING ? (
                  ...
                  <div className="space-y-6 text-white/60 leading-relaxed text-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                       <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-center">
                          <p className="text-red-500 font-bold mb-1">Red Channel</p>
                          <p className="text-[10px] uppercase text-white/30">Opposes Green</p>
                       </div>
                       <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-center">
                          <p className="text-blue-500 font-bold mb-1">Blue Channel</p>
                          <p className="text-[10px] uppercase text-white/30">Opposes Yellow</p>
                       </div>
                       <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-center">
                          <p className="text-white font-bold mb-1">Light Channel</p>
                          <p className="text-[10px] uppercase text-white/30">Opposes Dark</p>
                       </div>
                    </div>
                  </div>
                </section>
                    ...
                    <div className="space-y-6">
                      <div className="group p-6 bg-white/5 rounded-[2rem] border border-white/10 hover:border-purple-500/30 transition-all">
                        ...
                        <p className="text-sm text-white/50 leading-relaxed">
                          ...
                          <strong> L-cones</strong> react to long-wave (red), <strong>M-cones</strong> to medium-wave (green), and <strong>S-cones</strong> to short-wave (blue). 
                          Staring at a saturated color "bleaches" these proteins, temporarily rendering the cells unresponsive.
                        </p>
                      </div>
                      <div className="group p-6 bg-white/5 rounded-[2rem] border border-white/10 hover:border-purple-500/30 transition-all">
                        <h4 className="text-white font-bold mb-3 flex items-center gap-2">
                        ...
                        <p className="text-sm text-white/50 leading-relaxed">
                          Before signals reach the brain, they are pre-processed in the <strong>Ganglion cells</strong>. 
                          The brain doesn't see pure color; it sees the <em>difference</em> between cone outputs. 
                          Afterimage persistence is the time it takes for these cells to return to chemical equilibrium.
                        </p>
                      </div>
                    </div>
                    
                    <div className="relative flex items-center justify-center p-8 bg-black rounded-[3rem] border border-white/5">
                       <div className="space-y-8 text-center">
                          <div className="space-y-1">
                            <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Neural Pathway</p>
                            <p className="text-xl font-bold">Retina → Optic Nerve → V1 Cortex</p>
                          </div>
                          <div className="w-px h-16 bg-gradient-to-b from-indigo-500 to-transparent mx-auto"></div>
                          <p className="text-xs text-white/40 max-w-xs mx-auto italic">
                            The visual cortex (V1) interprets the lack of signal from fatigued cones as the presence of the opposing color. 
                            This is a fundamental mechanism of sensory homeostatis.
                          </p>
                       </div>
                    </div>
                  </div>
          ...
          <div className="max-w-4xl mx-auto py-12 space-y-16 animate-in zoom-in-95 duration-500">
            <div className="text-center space-y-8">
              <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-[10px] font-bold uppercase tracking-widest">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-20">
           <div className="space-y-6">
              ...
              <p className="text-xs text-white/30 leading-relaxed">
                ...
                Designed for students of neural science and visual physics.
              </p>
           </div>
           <div className="space-y-4">
              <h6 className="text-[10px] font-bold uppercase text-white/20 tracking-[0.3em]">Quick Navigation</h6>
              <ul className="space-y-2 text-xs text-white/40">
                <li><button onClick={() => navigateToSection('theory')} className="hover:text-indigo-400 transition-colors">Theory Overview</button></li>
                <li><button onClick={() => navigateToSection('anatomy')} className="hover:text-indigo-400 transition-colors">Ocular Anatomy</button></li>
                <li><button onClick={startConfig} className="hover:text-indigo-400 transition-colors">Start Experiment</button></li>
              </ul>
           </div>
           <div className="flex flex-col md:items-end justify-between opacity-20 text-[9px] uppercase font-mono tracking-[0.4em]">
              <div className="text-right">
                <p>Scientific Exhibit V1.6.1</p>
                <p>Node ID: {DataService.getParticipantId()}</p>
                {isAdmin && <p className="text-red-500 font-bold mt-1">ADMIN_MODE_ACTIVE</p>}
              </div>
